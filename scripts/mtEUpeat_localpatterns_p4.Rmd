...
id='EU_1131548'
#all_swind_ids=c('EU_1649895-0000000000-0000000000','EU_1649895-0000000000-0000004864','EU_1649976-0000000000-0000000000','EU_1649976-0000000000-0000004864')
#all_swind_ids=c('EU_1936806','EU_1623481','EU_1623480', 'EU_1755161', 'EU_2331582','EU_1131547','EU_1131548', 'EU_1623525')
all_swind_ids=c('EU_1936807','EU_1936819','EU_1936813','EU_1936832')
#all_swind_ids=swind_overlap_id_proj$ID
all_swind_ids=all_swind_ids[-26]
all_swind_ids=all_swind_ids[-26]
new_sights_BR=st_read('/home/laurag/Arbeit/wwu/R/workspaces/satellite_ts_R/random_sample_analysis/peats_swindles/selected_sites_plots/BjornRobroek/InteraCt_plus.shp')
new_sights_BR=st_read('/home/laurag/Arbeit/wwu/R/workspaces/satellite_ts_R/random_sample_analysis/ndwi_tif_after_snow/selected_sites_final_paper.shp')
colnames(new_sights_BR)
all_swind_ids=c(new_sights_BR$ID)
all_swind_ids=all_swind_ids[c(1:12,14:length(all_swind_ids))]
which(all_swind_ids=='EU_1623525')
#EU_1623556_1 & EU_1623556_2
id='EU_1623556'
lapply(all_swind_ids, function(id){
#lapply(all_swind_ids[c(1:12,14:length(all_swind_ids))], function(id){
#lapply(seq(12), function(l){
  #plot_sf=st_as_sf(slope_filtered[[l]])
  #print(month_names_full[l])
peat_rast=rast(paste0(path_peat_swind_tifs, id, '.tif'))
peat_rast_summer_mean=mean(subset(peat_rast,grep('Sslope_jun',names(peat_rast), value = T)),
    ...
    #forest_mask_na=as.numeric(forest_mask_na)
    #plot(forest_mask_na)
    #peat_rast_Sslope=subset(peat_rast, grep('Sslope', names(peat_rast), value = T))
    #random_points_within_peats=terra::intersect(vect(plot_sf),ext(peat_rast))
    #random_points_within_peats_sf=st_as_sf(random_points_within_peats)
    ...
    #ggplot() + geom_spatraster(data=forest_mask_na_resamp_crop) +geom_sf(data=peat_shape,colour= 'black', fill='#FF000000', size=1) 
    water_mask_na_resamp=resample(water_mask_crop, peat_rast_summer_mean_proj, method='near')
    #plot(water_mask_na_resamp_crop)
...
#ggplot() + geom_spatraster(data=peat_rast_summer_mean_masked) +geom_sf(data=peat_shape,colour= 'black', fill='#FF000000', size=1)
peat_rast_summer_mean_proj=project(peat_rast_summer_mean_plot, crs('epsg:4326'))

    #swind_point=all_peats_swind_overlap[which(all_peats_swind_overlap$ID == id),]
    #swind_point_spat_vec=vect(swind_point)
    #pixel_extracted=terra::extract(peat_rast, swind_point_spat_vec, bind = TRUE)
...
ggplot() +
     ...
     #geom_sf(data=swind_point,size =3, colour='black')+   
     geom_sf(data=peat_shape,colour= 'black', fill='#FF000000', size=1)+    
     #geom_sf(data=random_points_within_peats_sf, aes_string(fill=colnames(as.data.frame(random_points_within_peats_sf))[1]), size =2,shape = 21) +
     #scale_fill_stepsn(colours = pal(7), limits= c(-0.006,0.006), breaks =c(-0.006,-0.0015,0,0.0015,0.006)) +
     #geom_sf(data=swind_point, aes(colour=factor(LSts_trend_reconstructed)),size =2)+
     #scale_colour_manual(values=c('-1'='#3081e8', '0'='#ffffff', '1'='#ff3000'))+
     #labs(colour = "Reconstructed Trend")+ 
     #ggtitle(id) + 
    myTheme
...
setwd('/home/laurag/Arbeit/wwu/R/workspaces/satellite_ts_R/random_sample_analysis/peats_swindles/selected_sites_plots/BjornRobroek/plots_indiv_sites/')
getwd()
id='EU_2297536'
all_swind_ids=c('EU_2331582')
p_val_filter=TRUE
#all sslopes selected sites
lapply(all_swind_ids, function(id){
    ...
    if(p_val_filter==T){
    #peat_rast_pval=subset(peat_rast, grep('p_val', names(peat_rast), value = T))
    #peat_rast_count=subset(peat_rast, grep('count', names(peat_rast), value = T))
    ...
    forest_mask_na_resamp_crop=as.numeric(forest_mask_na_resamp_crop)
    #plot(forest_mask_na_resamp_crop)
...
#rename_lookup_v$new[match(names(peat_rast_Sslope_proj_rename), rename_lookup_v$old)]
...
ggplot() +
     ...
     facet_wrap(~lyr, ncol = 3) +
     scale_fill_stepsn(colours = pal(7), limits= c(-0.006,0.006), breaks =c(-0.006,-0.003,-0.001,0,0.001,0.003,0.006),na.value='#00000000') + 
     #geom_sf(data=swind_point, aes(colour =factor(trend_sign)))+
     #scale_colour_manual(values=c('-1'='darkblue', '0'='#7c7541', '1'='darkred'))+
     geom_sf(data=peat_shape,colour= 'black', fill='#FF000000', size=1)+ 
...
```{r, eval = T, echo = F}
# NDWI raster timeseries
###line plots ts_plus_trend
...
path_plots='/home/laurag/Arbeit/wwu/R/workspaces/satellite_ts_R/random_sample_analysis/peats_swindles/selected_sites_plots/BjornRobroek/plots_indiv_sites/significance_filtered/'

trend_path='/home/laurag/Arbeit/wwu/R/workspaces/satellite_ts_R/random_sample_analysis/50singlepeats/exported_GEE/InterAct/'
...
new_sights_BR=st_read('/home/laurag/Arbeit/wwu/R/workspaces/satellite_ts_R/random_sample_analysis/ndwi_tif_after_snow/selected_sites_final_paper.shp')
all_swind_ids=c(new_sights_BR$ID)
all_swind_ids=all_swind_ids[7]
l=1

lapply(seq(length(all_swind_ids)), function(l){
ts_list=list.files(paste0(ts_rast_path,all_swind_ids[l]), pattern = '*.tif$', full.names=TRUE)
ts_list_rast=lapply(ts_list, rast)
stack_rast=rast(ts_list_rast)
point_id=points_extr %>% filter(ID == all_swind_ids[l])
...
#plot all NDWI time series
#ggplot(ts_extracted_pt)+
  #geom_line(aes(x=Year, y=NDWI, color=Month))

#only time series with significant trends
#p_sign_max_freq=filtered_yr_sf[unlist(st_intersects(max_freq_point, filtered_yr_sf)),]  
#month_df=data.frame(name=c(month.name),nr=c(seq(12)))
#filtered_m_nr= month_df %>% filter(name %in% p_sign_max_freq$Month)
###--------------------------------###
### iterate over months
###--------------------------------###
#month_i = unique(ts_extracted_pt$Month)[6]
...
#list_trend_model_data=lapply(as.factor(seq(5,10)), function(month_i){
#print(month_i)
#sig_mon=filtered_m_nr$nr[month_i]
#ts_extracted_pt_sign = ts_extracted_pt %>% filter(Month %in% filtered_m_nr$nr)
...
#if(as.numeric(as.character(month_i)) ==10){
...
#lot(filtered_ts_mon$Year, filtered_ts_mon$NDWI)
#lines(filtered_ts_mon$fit, col = 'red')
#filtered_month_single=
...
ggplot(filtered_ts_mon[!is.na(filtered_ts_mon$NDWI), ])+
  ...
  annotate(geom="text", x=2013, y=-0.5, label=paste0('Sens Slope [NDWI/yr] = ',round(sens_USGS_sig_mon$coefficients[2], digits=5)), size=7)+
  geom_hline(yintercept=0.0, color='#323233')+
  geom_point(aes(x=Year, y=NDWI, color = 'NDWI'), size = 2)+
  ggtitle(month.name[as.numeric(as.character(month_i))])

ggsave(file = paste0(path_plots, 'line_plots/lplot_trendts_', all_swind_ids[l], '_', month.name[as.numeric(as.character(month_i))], '.svg'),
   width = 10, # The width of the plot in inches
   height = 5)  
}
